...
def send_alert(hostname, ip, alert_type, message, info=""):
    try:
        ...
        if response.status_code != 200:
            print(f"[ALERT ERR] {response.status_code}")
...
def run():
    ...
    while True:
        try:
            ...
            if response.status_code == 200:
                print(
                    f"[OK] CPU:{cpu}% RAM:{ram.percent}% "
                    f"USB:{len(curr_usb)} PORTS:{len(open_ports)} "
                    f"PROC:{len(processes)} IPs:{len(active_ips)} | {hostname}"
                )
            else:
                print(f"[ERR] Server {response.status_code}: {response.text[:140]}")
